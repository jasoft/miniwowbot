...
class EmulatorConnectionManager:
    ...
    def _decode_output(self, raw_output: bytes | str | None) -> str:
        """安全解码子进程输出。

        Args:
            raw_output: 子进程返回的标准输出或错误输出。

        Returns:
            解码后的文本；当无法精确解码时，使用替换字符兜底。
        """
    ...
    def _is_local_port_open(self, port: int, timeout: float = 0.5) -> bool:
        """检查本机端口是否已监听。
    ...
    def connect(self, emulator: str) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                return False
            output = stdout_text.strip().lower()
    ...
    def test_connection(self, emulator: str) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return True
            self.logger.warning(f"[ADB] 连接测试失败: {emulator}, 输出: {stderr_text}")
            return False
        except Exception as exc:
    ...
    def _normalize_emulator(self, name: str) -> str:
        ...
        if (
            ...
            if self._is_local_port_open(port):
                normalized = f"127.0.0.1:{port}"
                self.logger.info(f"[ADB] 检测到本地端口 {port} 已监听，改用 {normalized}")
                return normalized
