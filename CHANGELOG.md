...
改进 `cron_launcher.py`，添加自动重试机制。当某个配置的脚本运行失败时，会自动重新运行该脚本直到成功，然后继续下一个配置。实现完全无人值守的自动化运行。

### 主要改进

1. **新增 `run_dungeons_with_retry()` 函数**
    - 每个配置最多重试 3 次
    - 第 1 次失败后等待 10 秒重试
    - 第 2 次失败后等待 20 秒重试
    - 第 3 次失败后等待 30 秒重试
...
运行配置 2 (mage_alt, 127.0.0.1:5565)
  ├─ 第 1 次尝试 → 失败
  ├─ 等待 10 秒
  ├─ 第 2 次尝试 → 失败
  ├─ 等待 20 秒
...
## [功能] 脚本失败自动重试机制 - 2025-11-04

### 功能描述

改进 `run_all_dungeons.sh` 脚本，添加自动重试机制。当某个角色的脚本运行失败时，会自动重新运行该脚本直到成功，然后继续下一个角色。
...
**`run_all_dungeons.sh`** (修改)

- 新增 `run_character()` 函数中的重试循环
- 添加指数退避等待时间（10秒、20秒、30秒）
- 更新帮助信息说明重试机制
- 简化主函数中的错误处理逻辑
...
# 运行特定角色（失败自动重试）
./run_all_dungeons.sh warrior mage

# 在指定模拟器上运行（失败自动重试）
./run_all_dungeons.sh --emulator 127.0.0.1:5555
```
