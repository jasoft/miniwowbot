...
1. **Loki 日志系统** - 在 Loki 中作为 `logger` 标签，用于日志查询和过滤
2. **日志来源识别** - 在 Loki 中区分不同模块或组件的日志
3. **日志记录器层级** - 支持日志记录器的层级结构

**重要说明**：`name` 参数**仅在 Loki 中有意义**，在 console 输出中不会显示（除非在日志格式中明确添加 `%(name)s`）。
